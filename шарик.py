...
def draw_text(surf, text, size, x, y): # функция для печать на экране очков
    font = pygame.font.Font(font_name, size) #подискиваем необходимы шрифт на компьютере и говорим размер текста
    text_surface = font.render(text, True, WHITE) #создает поврехноть, на которой написан текст
    text_rect = text_surface.get_rect() #методо возвращает объект типа Rect, по умолчанию (0, 0)
    text_rect.midtop = (x, y) #задаются координаты верх середины
    surf.blit(text_surface, text_rect) #вывод на экран

def show_go_screen():
    screen.blit(background_image, (0, 0))
    draw_text(screen, "Шарик", 100, WIDTH / 2, HEIGHT / 30)
    draw_text(screen, "Нажмите какую-то кнопку на клавиатуре для начала игры", 22, WIDTH / 2, HEIGHT / 2)
    draw_text(screen, "Press a key to begin", 20, WIDTH / 2, HEIGHT * 2 / 3)
    pygame.display.flip()
    waiting = True
    while waiting:  # миниатюрный цикл игры
        clock.tick(FPS)
